...
public class Broadcaster : IEntity<int>, IExternalRef
{
    ...
    /// <summary>
    /// The stored mark artwork should use: the dark variant first — ESPN's networks publish a
    /// light-on-dark mark that reads on a team-coloured card — then the default, then whatever
    /// exists. <see cref="LogoSelection.ForBroadcaster"/> applies the same order to a source's
    /// candidates before anything is downloaded.
    /// </summary>
